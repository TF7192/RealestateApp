...
// admin. Public read, OWNER-only mutations. Members cascade from the
// FK ON DELETE CASCADE — the tests lock that behaviour in so a future
// migration doesn't drop the cascade silently.
describe('NeighborhoodGroup admin', () => {
  it('H — OWNER can create a group with members; GET includes them', async () => {
  ...
  it('Az — plain AGENT is rejected on POST / PATCH / DELETE', async () => {

// Hebrew copy for the landing page. One-file source of truth so a
// Hebrew-fluent reviewer can edit the whole marketing surface without
// hunting through JSX. Open items flagged inline as TODO(landing).
//
// Kept in sync with landing/content/copy.he.ts at the repo root; the
// frontend bundle imports this .js copy so we don't bring TypeScript
// into a JSX-only codebase.

export const copy = {
  meta: {
    title: 'CRM לסוכני נדל״ן · אפליקציה לאייפון ואנדרואיד · Estia',
    description:
      'CRM מודרני לסוכני נדל״ן בישראל: לקוחות, נכסים, פגישות, AI לתיאורי נכסים והתאמת מתעניינים — בנייד. 30 יום חינם, בלי כרטיס אשראי.',
    og_alt: 'Estia — מערכת ניהול נדל״ן לסוכנים, עם אפליקציה לאייפון ולאנדרואיד.',
    canonical: 'https://estia.co.il/',
  },

  nav: {
    logo: 'Estia',
    links: [
      { id: 'features', label: 'תכונות' },
      { id: 'pricing',  label: 'מחירים' },
      { id: 'faq',      label: 'שאלות נפוצות' },
    ],
    login_cta:   'התחברות',
    primary_cta: 'התחלה חינם',
    mobile_menu_open:  'פתיחת תפריט',
    mobile_menu_close: 'סגירת תפריט',
  },

  hero: {
    eyebrow: 'ה-CRM לסוכני הנדל״ן בישראל',
    h1_line_1: 'ה-CRM של סוכני הנדל״ן.',
    h1_line_2: 'כולו בנייד שלך.',
    sub:
      'ניהול לקוחות, נכסים, פגישות ומתעניינים — מהמקום שבו אתם באמת עובדים. ' +
      'עם AI שמבין נדל״ן, ו-30 יום חינם בלי כרטיס אשראי.',
    primary_cta: 'התחלה חינם ל-30 יום',
    secondary_cta: 'כבר יש לי חשבון',
    trust: ['בלי כרטיס אשראי', 'ביטול בכל רגע', 'תמיכה בעברית'],
    screenshot_alt: 'מסך הבית של Estia: רשימת המתעניינים, הנכסים והיומן להיום.',
    app_store_label:   'הורדה מ-App Store',
    google_play_label: 'להורדה ב-Google Play',
    store_disabled_note: 'האפליקציות לאייפון ואנדרואיד בדרך — משתמשים רשומים יקבלו גישה ראשונים.',
  },

  // Dedicated AI spotlight block (between Features and MobileApp). The
  // Cream-&-Gold redesign pulls the AI from "just another card" into
  // its own hero-sized section so the premium-only nature of the
  // feature is unmissable.
  ai: {
    eyebrow: 'Estia AI · Premium',
    title: 'ה-AI הראשון שמבין באמת נדל״ן ישראלי',
    sub:
      'מתאר נכסים כמו קופירייטר מנוסה. מתאים מתעניינים לנכסים בעצמו. ' +
      'חוסך שעות של "מי מתאים למה". כלול רק במסלול Premium.',
    premium_chip: 'PREMIUM ONLY · ✦',
    premium_note: 'כלול רק במסלול Premium',
    actions: [
      { label: 'כותב תיאור שיווקי', hint: '≈ 22 שניות' },
      { label: 'מתאים מתעניין לנכס',    hint: 'דירוג 0–100' },
      { label: 'סיכום פגישה',       hint: 'מהקלטה' },
    ],
    playground_title: 'Estia AI Playground',
    playground_badge: 'v2.4 · נדל״ן',
    playground_fields_label: 'נתוני נכס',
    playground_fields: [
      { k: 'סוג',  v: 'דירת 4 חד׳' },
      { k: 'עיר',  v: 'רמת גן' },
      { k: 'קומה', v: '5/7' },
      { k: 'שטח',  v: '104 מ״ר' },
    ],
    playground_output_label: 'תיאור שיווקי · עברית',
    playground_idle: 'לחצו על "צור תיאור"…',
    playground_sample:
      'דירת 4 חדרים מוארת ומרווחת ברחוב שקט בלב רמת גן, קומה 5 עם מעלית, ' +
      'מרפסת שמש 12 מ״ר פונה לפארק, חניה ומחסן. סמוכה לבתי ספר ותחבורה.',
    playground_cta: 'צור תיאור חדש',
  },

  features: {
    section_eyebrow: 'תכונות',
    section_title: 'כל הכלים שסוכן נדל״ן באמת צריך',
    section_sub:
      'המערכת נבנתה מתוך עבודה אמיתית בשטח — כל מה שאתם עושים ביום-יום, ' +
      'במקום אחד, גם בדסקטופ וגם בנייד.',
    cards: [
      {
        key: 'ai',
        title: 'AI שמבין נדל״ן',
        body:
          'תיאור נכס מקצועי בלחיצה — AI כותב עבורכם טקסט שיווק מושלם מהנתונים. ' +
          'התאמת מתעניינים חכמה שמראה לכם איזה לקוח מתאים לאיזה נכס, אוטומטית.',
      },
      {
        key: 'leads',
        title: 'ניהול לקוחות ומתעניינים',
        body:
          'מעקב מלא על כל מתעניין — מהשיחה הראשונה ועד העסקה. חם/פושר/קר, ' +
          'תזכורות חכמות, והיסטוריה מלאה לכל לקוח במקום אחד.',
      },
      {
        key: 'properties',
        title: 'נכסים, בעלים ובלעדיויות',
        body:
          'תיק נכס מלא: תמונות, בעלים, מסלול שיווק, זכויות חתומות והסכמי בלעדיות. ' +
          'תמיד זמין לשליחה ללקוח בקישור אחד.',
      },
      {
        key: 'calendar',
        title: 'יומן פגישות חכם',
        body:
          'פגישה חדשה שואבת את הלקוח, הנכס והכתובת אוטומטית. ' +
          'סנכרון דו-כיווני ליומן גוגל — בנייד ובדסקטופ.',
      },
      {
        key: 'calculator',
        title: 'מחשבון עמלות ועמלת תיווך',
        body:
          'נטו למוכר או מחיר סגירה — דינמי, עם מע״מ, שכר טרחת עו״ד ועלויות נוספות. ' +
          'שיתוף תוצאה ללקוח בקישור אחד.',
      },
      {
        key: 'mobile',
        title: 'אפליקציה לאייפון ולאנדרואיד',
        body:
          'כל מה שיש באתר — גם בכיס שלכם. שיחות, WhatsApp ופגישות ' +
          'ישירות מהאפליקציה. עובדת במעלית, בנהיגה, בפגישה.',
      },
    ],
    more_features_title: 'וגם:',
    more_features: [
      'תבניות WhatsApp חכמות לשליחה מהירה',
      'העברת נכסים ובלעדיות בין סוכנים',
      'דף נכס ציבורי ללקוחות בקישור אחד',
      'ייבוא נתונים מ-Excel',
      'תזכורות ומשימות אוטומטיות',
      'תיוג וסינון חכם של לקוחות ונכסים',
      'דו״חות ביצועים לסוכן ולמשרד',
      'תמיכה בעברית מלאה, RTL-first',
    ],
  },

  mobile: {
    section_eyebrow: 'האפליקציה',
    section_title: 'כל המתעניינים שלכם — כבר עכשיו בכיס שלכם',
    section_sub:
      'אפליקציה נטיבית לאייפון ולאנדרואיד. עובדת גם במעלית, ' +
      'גם בנהיגה, גם באמצע פגישה.',
    bullets: [
      'הודעות WhatsApp, שיחות ופגישות — ישירות מהאפליקציה.',
      'התראות חכמות שלא יגרמו לכם לפספס מתעניין חם.',
      'תמונות, מיקום, ניווט — הכול בנגיעה אחת.',
      'סנכרון מיידי עם הדסקטופ — ממשיכים מאיפה שעצרתם.',
    ],
    qr_label_desktop: 'סרקו כדי להוריד',
    screenshot_alt: 'מסך המתעניינים באפליקציית Estia — לחיצה אחת על מתעניין לפתיחת כרטיס מלא.',
  },

  how: {
    section_title: 'בעוד 2 דקות — אתם כבר עובדים',
    section_sub: 'הכנה זה לא מקצוע. עבודה — כן.',
    steps: [
      { n: 1, title: 'נרשמים בדקה',            body: 'אימייל וסיסמה או התחברות עם Google. בלי כרטיס אשראי, בלי שיחת מכירה.' },
      { n: 2, title: 'מייבאים או מתחילים נקי', body: 'ייבוא מ-Excel, או הקלדת הנכסים והלקוחות הראשונים — הבחירה שלכם.' },
      { n: 3, title: 'עובדים. 30 יום חינם.',    body: 'ביטול בכל רגע, בלי התחייבות, בלי הפתעות.' },
    ],
  },

  pricing: {
    section_eyebrow: 'מחירים',
    section_title: 'מחיר ברור. הכול כלול.',
    section_sub: 'התוכנית החינמית פתוחה לכולם — בלי כרטיס אשראי.',
    paid_only_via_support: 'שדרוג לתוכניות בתשלום מתבצע כרגע דרך הצוות שלנו במייל support@estia.co.il',
    cycle_toggle: { monthly: 'חיוב חודשי', yearly: 'חיוב שנתי — חודשיים מתנה' },
    yearly_hint: 'חודשיים מתנה בתשלום שנתי',
    vat_note: 'כל המחירים כוללים מע״מ',
    currency: '₪',
    tiers: [
      {
        key: 'free',
        name: 'חינם',
        lead: 'להתחיל מיד — בלי כרטיס אשראי',
        free: true,
        recommended: false,
        bullets: [
          'עד 10 נכסים פעילים',
          'עד 25 לקוחות/מתעניינים',
          'אפליקציה לאייפון ואנדרואיד',
          'סנכרון ליומן גוגל',
          'תמיכה בעברית',
        ],
        cta: 'התחלה חינם',
      },
      {
        key: 'starter',
        name: 'בסיס',
        lead: 'לסוכן נדל״ן עצמאי',
        monthly: 99,
        yearly:  990,
        recommended: false,
        bullets: [
          'עד 50 נכסים פעילים',
          'עד 100 לקוחות/מתעניינים',
          'AI לתיאורי נכסים ולהתאמת מתעניינים',
          'אפליקציה לאייפון ואנדרואיד',
          'סנכרון ליומן גוגל',
          'תמיכה בעברית',
        ],
        cta: 'שדרוג דרך התמיכה',
        cta_href: 'mailto:support@estia.co.il?subject=Estia%20%E2%80%94%20%D7%A9%D7%93%D7%A8%D7%95%D7%92%20%D7%9C%D7%AA%D7%95%D7%9B%D7%A0%D7%99%D7%AA%20%D7%91%D7%A1%D7%99%D7%A1',
      },
      {
        key: 'pro',
        name: 'מקצועי',
        lead: 'למשרד או לסוכן נדל״ן בנפח גבוה',
        monthly: 249,
        yearly:  2490,
        recommended: true,
        recommended_badge: 'הכי פופולרי',
        bullets: [
          'נכסים ולקוחות ללא הגבלה',
          'AI לתיאורי נכסים ולהתאמת מתעניינים — ללא הגבלה',
          'ייבוא מ-Excel ומיבואי צד ג׳',
          'יומן מתקדם + תבניות WhatsApp',
          'מחשבון עמלות ועמלת תיווך',
          'דף נכס ציבורי + הודעות שיתוף מעוצבות',
          'העברות נכסים בין סוכנים',
          'עדיפות תמיכה',
        ],
        cta: 'שדרוג דרך התמיכה',
        cta_href: 'mailto:support@estia.co.il?subject=Estia%20%E2%80%94%20%D7%A9%D7%93%D7%A8%D7%95%D7%92%20%D7%9C%D7%AA%D7%95%D7%9B%D7%A0%D7%99%D7%AA%20%D7%9E%D7%A7%D7%A6%D7%95%D7%A2%D7%99',
      },
    ],
    trust: [
      'בלי כרטיס אשראי לניסיון',
      'ביטול בכל רגע',
      'תמיכה בעברית',
      'שרתים באירופה (עומד בתקני GDPR)',
    ],
  },

  faq: {
    section_eyebrow: 'שאלות',
    section_title: 'שאלות ששואלים אותנו',
    items: [
      { q: 'האם באמת אין צורך בכרטיס אשראי?',        a: 'נכון. רושמים אימייל וסיסמה, נכנסים, עובדים. כרטיס אשראי מוזן רק אם החלטתם להמשיך אחרי 30 יום.' },
      { q: 'מה קורה אחרי 30 יום?',                    a: 'יום לפני סוף התקופה נשלח אימייל. אם לא בחרתם מסלול בתשלום, החשבון עובר למצב צפייה בלבד — הנתונים נשמרים ותוכלו לחזור בכל רגע.' },
      { q: 'האם האפליקציה עובדת על אייפון ואנדרואיד?', a: 'כן — אפליקציה נטיבית לשני המכשירים. האפליקציות בתהליך פרסום בחנויות; מי שנרשם עכשיו מקבל גישה ראשון.' },
      { q: 'מה ה-AI בדיוק עושה?',                     a: 'שני דברים: כותב עבורכם תיאור נכס שיווקי מתוך הנתונים שהזנתם, ומציג התאמה חכמה בין לקוחות לנכסים — חוסך שעות של "מי מתאים למה".' },
      { q: 'האם אפשר לייבא נתונים מ-Excel?',          a: 'כן. ייבוא מ-Excel בקליק — נכסים, לקוחות ובעלים עולים ביחד בלי העתק-הדבק.' },
      { q: 'האם הנתונים שלי מאובטחים?',                a: 'הנתונים מוצפנים במעבר ובמנוחה, נשמרים על שרתים באירופה (AWS eu-north-1), וגיבויים אוטומטיים נשמרים 14 יום. אתם הבעלים של המידע — ניתן לייצא או למחוק אותו בכל רגע.' },
      { q: 'מתאים גם לסוכן עצמאי או רק למשרדים?',     a: 'שניהם. המסלול "בסיס" מתאים לסוכן עצמאי; "מקצועי" מיועד למשרדים ולסוכנים בנפח גבוה.' },
    ],
  },

  final_cta: {
    h2: 'מוכנים לנהל את העסק שלכם אחרת?',
    sub: '30 יום חינם. בלי כרטיס אשראי. בלי התחייבות.',
    primary_cta: 'להתחלה חינם',
    secondary_label: 'או הורידו את האפליקציה:',
  },

  footer: {
    tagline: 'Estia — ה-CRM לסוכני הנדל״ן בישראל.',
    columns: [
      {
        title: 'מוצר',
        links: [
          { label: 'תכונות',        href: '#features' },
          { label: 'מחירים',        href: '#pricing' },
          { label: 'שאלות נפוצות', href: '#faq' },
          { label: 'התחברות',       href: '/login' },
        ],
      },
      {
        title: 'חברה',
        links: [
          { label: 'צור קשר',       href: 'mailto:hello@estia.co.il' },
          { label: 'תנאי שימוש',    href: '/terms' },
          { label: 'מדיניות פרטיות', href: '/privacy' },
        ],
      },
    ],
    copyright: '© 2026 Estia · כל הזכויות שמורות',
  },

  shared: {
    currency_suffix: ' / חודש',
    vat_inclusive: 'כולל מע״מ',
    anchor_to_pricing: 'לראות מחירים',
    coming_soon: 'בקרוב',
  },
};

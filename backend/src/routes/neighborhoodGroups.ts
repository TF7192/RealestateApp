...
// the G1 Neighborhood dictionary. OWNERs curate groups; agents
// consume them read-only. Keeps the public GET consistent with the
// G1 /api/neighborhoods shape so the frontend can pair them freely.
...
export const registerNeighborhoodGroupRoutes: FastifyPluginAsync = async (app) => {
  ...
  // OWNER-gated writes. Same stance as the G1 write path — the lookup
  // is office-global, so agents don't mutate it.
  app.post('/', { onRequest: [app.requireOwner] }, async (req, reply) => {
  ...
  app.patch('/:id', { onRequest: [app.requireOwner] }, async (req, reply) => {
  ...
  app.delete('/:id', { onRequest: [app.requireOwner] }, async (req, reply) => {
    const { id } = req.params as { id: string };
    const existing = await prisma.neighborhoodGroup.findUnique({ where: { id } });
    if (!existing) return reply.code(404).send({ error: { message: 'Not found' } });
